...
<head>
    ...
    <style>
        ...
        .card {
            border: 3px solid rgb(237 242 249);
        }

        .card-footer {
            background-color: rgb(10 150 157);
    ...
    <div class="container" style="padding:0px !important; margin-bottom: 60px">
        ...
        <div class="container ">
            <div class="row">
                ...
                <div class="col-lg-9 ">
                    <div class="row row-cols-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5 row-cols-xxl-5  gx-0">
                        <?php if ($lowongan) {
                            foreach ($lowongan as $row) { ?>
                                <div class="card h-100 mb-2">
                                    <a href="<?= base_url('gform_detail/' . $row['id']) ?>" class="text-decoration-none" onClick="loading()">
                                        <img src="<?= base_url('lowongan/' . $row['banner_lowongan']) ?>" class="card-img-top" alt="mobil">
                                        <div class="card-body p-2">
                                            <p class="card-title card-judul-lowongan"><?= $row['nama_lowongan'] ?></p>
                                            <p class="card-text mb-0 text-body"><i class="bi bi-people-fill"></i> Kuota: <?= $row['kuota_lowongan'] ?> Kandidat</p>
                                            <p class="card-text mb-0 text-body"><i class="bi bi-calendar-check-fill"></i> <?= date('d-M', strtotime($row['dibuka_lowongan'])) ?> s/d <?= date('d-M-Y', strtotime($row['ditutup_lowongan'])) ?></p>
                                            <p class="card-text mb-0 text-body"><i class="bi bi-flag-fill"></i> Negara: <?= $row['negara_lowongan'] ?></p>
